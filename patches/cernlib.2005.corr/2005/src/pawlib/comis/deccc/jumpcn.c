...
/*
 * function jumpcadd (used by cskcal.F):
 * required for LP64 architectures to modify arguments containing an externals
 * which are stored relative to the address of jumpad_ in the comis package
 */

#define jumpcadd jumpcadd_
extern int jumpad_();
int  jumpcadd(iaddr)
     int *iaddr;
{
     int iret;
     iret = *iaddr + (long)jumpad_;
#if defined (__ia64__)
/*   For Itanium architectures this value has to be flagged (a little bit dirty).
     It is required in cscal_lp64.h to put it in shared memory region (region 1)    
*/
     if (iret >= 0x80000000) 
     {
      printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
